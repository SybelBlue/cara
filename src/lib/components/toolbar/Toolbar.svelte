  ...
  <div class="w-full px-4 py-3 flex items-center justify-between">
    ...
    <nav class="flex-1 flex flex-row-reverse gap-4">
      {#if currentDeck.length > 0}
        ...
        <ToolbarButton bind:toggled={showTests} label="show design tests">
          {#snippet inner(toggled)}
            <svg
              class:open={toggled}
        ...
        {#if commits.length > 0}
          <ToolbarButton
            bind:toggled={showTimeline}
            label={(showTimeline ? 'hide' : 'show') + ' timeline'}
          >
            {#snippet inner(toggled)}
              <svg
                class:open={toggled}
        ...
        <ToolbarButton bind:toggled={showSubmissionModal} label="download state">
          {#snippet inner(toggled)}
            <svg
              class:open={toggled}
              xmlns="http://www.w3.org/2000/svg"
...
{#if showSubmissionModal}
  <SubmissionModal {submissionYaml} close={() => (showSubmissionModal = false)} />
{/if}
